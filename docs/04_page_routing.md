...
| `/sign-in`         | User login page.                  | `POST /api/v1/login`                                           | Public    |
| `/sign-up`         | User registration page (Assumed). | `POST /api/v1/users` (Assumed)                                 | Public    |
...
1.  **Public Routes**:
    -   The `/sign-in` and `/sign-up` pages will be directly accessible.
...
2.  **Protected Routes**:
    ...
    -   A `middleware.ts` file at the root (`src/middleware.ts`) will be responsible for checking for the `refresh_token` cookie to protect routes. If the cookie is missing, it will redirect to `/sign-in`.
